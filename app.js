$(function (start) {
    ...
    var move_right1 = false;  //second players paddle
    var move_left1 = false;

    var who_won;  //displays name of the winner of the game
    ...
    $(document).on('keydown', function (e) {
        var key = e.keyCode;
        if (key === 37 && move_left === false && game_over === false) { // Left arrow key
            move_left = requestAnimationFrame(left);
        } else if (key === 39 && move_right === false && game_over === false) { // Right arrow key
            move_right = requestAnimationFrame(right);
        } else if (key === 65 && move_left1 === false && game_over === false) {  // 'A' key
            move_left1 = requestAnimationFrame(left1);
        } else if (key === 83 && move_right1 === false && game_over === false) {  // 'S' key
            move_right1 = requestAnimationFrame(right1);
        }
    });
    ...
    $(document).on('keyup', function (e) {
        var key = e.keyCode;
        if (key === 37 && game_over === false) { // Left arrow key
            cancelAnimationFrame(move_left);
            move_left = false;
        } else if (key === 39 && game_over === false) { // Right arrow key
            cancelAnimationFrame(move_right);
            move_right = false;
        } else if (key === 65 && game_over === false) {  // 'A' key
            cancelAnimationFrame(move_left1);
            move_left1 = false;
        } else if (key === 83 && game_over === false) {  // 'S' key
    ...
    function collision($div1, $div2) {   //used to detect whether or not two divs collide or not
